...
def main():
    ...
    for epoch in range(epochs):
        ...
        for batch in training_loader:
            ...
            y_train = torch.LongTensor(np.argmax(y_train, axis=1))    # Fix extract feature to not store one hot target
        ...
        with no_grad():
            for batch in validation_loader:
                x_val, y_val = batch
                y_val = torch.LongTensor(np.argmax(y_val, axis=1))    # Fix extract feature to not store one hot target
        ...
        if patience_counter >= patience or epoch == epochs:
            ...
            if best_model is None:
                ...
                with no_grad():
                    for batch in testing_loader:
                        x_test, y_test = batch
                        y_test = torch.LongTensor(np.argmax(y_test, axis=1))    # Fix extract feature to not store one hot target
